...
class TablaVentas{
  ...
  public function mostrarTabla(){	
		...
		$stmt = Conexion::conectar()->prepare("SELECT * FROM compras WHERE estatus = 'abierto' ORDER BY id ASC");
		$stmt -> execute();
	...
	for($i = 0; $i < count($ventas); $i++){
		...
		if($venta != $ventas[$i]->id_venta){
			...
			/* foreach ($detalles as $key => $value) {
				...
				$total = $total + ($detalles->peso * $detalles->precio);
				...
				$informacion	 .= $detalles->title.';'.$detalles->peso.';'.$detalles->precio.';'.$detalles->id.'!';
