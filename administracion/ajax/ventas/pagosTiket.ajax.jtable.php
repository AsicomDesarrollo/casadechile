...
class TablaVentas{
  ...
  public function mostrarTabla(){	//mostrar todos los pagos de todos los tikets
    ...
    $stmt_total= Conexion::conectar()->prepare("select count(folio) tiket_pagos from tiket_pagos");
    $stmt_total-> execute();
		$query_total = $stmt_total -> fetchAll(PDO::FETCH_CLASS)[0]->tiket_pagos;
    ...
    //validado es el parametro que se actualiza con el corte de caja
		$stmt = Conexion::conectar()->prepare("SELECT * FROM tiket_pagos WHERE validado<>1 ".$orden." limit ".$limitInf.",". $limitSup);
		$stmt -> execute();
