...
class ModeloUsuarios{
	...
	*****************************/
	...
	static public function activarDesactivarUsuario($id,$estatus){
		...
		if($estatus==='habilitar'){
			$valorEstatus==1;
			$stmt = Conexion::conectar()->prepare("UPDATE empleados SET  estatus = '1' WHERE id = :id  ");
		}
		else
		if($estatus==='deshabilitar'){
			$valorEstatus==0;
			$stmt = Conexion::conectar()->prepare("UPDATE empleados SET  estatus = '0' WHERE id = :id  ");
		}

		$stmt->bindParam(":id", $id, PDO::PARAM_INT);
		//$stmt->bindParam(":valorEstatus", $valorEstatus , PDO::PARAM_STR);
